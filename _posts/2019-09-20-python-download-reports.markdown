...
作为一名理工 IT 男，后来又读了金融类的 MBA，那么利用计算机来解决金融方面的问题算是对“学以致用”这个词最好的注解了。该系列文章是我通过用 Python 对上市公司的企业价值分析的软件开发过程的一个整理与回顾，计划写四篇，包括从财务数据的获取、信息整理清洗与提取到最后的企业价值计算分析全过程。本文是第一篇，主要描述如何用 Python 获取企业年报数据。通过上市公司的定期财务报告，为接下来的企业价值分析提供分析数据。
